...
const ALL_NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    match: (pathname) => pathname === "/calendar" || pathname === "/en/calendar",
  },
  {
    href: "/opportunities",
    label: { ko: "기회", en: "Opportunities" },
    match: (pathname) => pathname === "/opportunities" || pathname.startsWith("/opportunities/"),
  },
  {
    href: "/popok-artist",
...
export default function Header() {
  ...
  const { language: storedLanguage, setLanguage } = useLanguage();
  const language: Language = pathname.startsWith("/en") ? "en" : storedLanguage;
      ...
      <div className="header-inner" style={{
        ...
        <div className="header-nav-links" style={{ display: "flex", justifyContent: "center", gap: "28px" }}>
          {NAV_ITEMS.map((item) => {
            ...
            const itemHref = language === "en" && item.href === "/calendar" ? "/en/calendar" : item.href;
            return (
              <Link
                key={item.href}
                href={itemHref}
