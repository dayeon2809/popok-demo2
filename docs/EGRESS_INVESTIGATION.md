...
## 남은 산출물 — 비개발자용 결과 보고서 (전달용)

작업이 전부 끝난 뒤 **별도 md로 작성한다.** 이 문서(조사 기록)와는 성격이 다르다.
...
**끝난 뒤에 쓰는 이유가 있다.** 이 보고서의 핵심은 "16.41 GB가 실제로 얼마가 되었는가"인데, 그 숫자는 배포 후 한 청구 주기를 돌려야 나온다. 추정치로 쓰면 보고서의 가치가 사라진다.

쓸 때 반드시 포함할 것 —

- 조치 **전후 사용량 실측** (16.41 GB → 실제 측정값)
- 왜 캐시로는 해결이 안 됐는지 (전송량은 배달된 바이트다)
- `cacheControl`은 Supabase 쪽 문제라 우리가 고칠 수 없었고, 프록시로 우회했다는 것
- 되돌릴 수 없어서 **일부러 하지 않은 것**(작업 3)과 그 이유
- 다음에 같은 일이 생기지 않게 하는 장치(회귀 테스트)가 무엇인지
