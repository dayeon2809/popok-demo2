...
## 비개발자용 전달 문서 → [EGRESS_WORK_SUMMARY.md](EGRESS_WORK_SUMMARY.md)

작성 완료. **무엇을 왜 했는지**는 지금 다 나와 있으므로 먼저 썼고, **결과 수치 한 줄만 배포 후에 채운다.**

이 문서(조사 기록)와는 성격이 다르다.

| | 이 문서 | 결과 보고서 |
|---|---|---|
| 독자 | 코드를 만지는 사람 | 비개발자. 팀·대표에게 전달된다 |
| 목적 | 재현하고 이어서 작업하기 | 무슨 문제였고 어떻게 해결했는지 이해시키기 |
| 담을 것 | 명령어, SQL, 파일:줄번호, 가설과 근거 | 원인, 조치, **조치 전후 실측 비교**, 남은 위험 |

배포 후 **한 청구 주기가 지나면 실측값을 채운다.** 지금은 "아직 배포 전"이라고 명시돼 있다. 추정치를 결과처럼 적지 않는다.

담은 것 —

- 원인과 각 작업의 **이유** (무엇을 했는지가 아니라 왜 했는지)
- 왜 캐시로는 해결이 안 되는지 (전송량은 배달된 바이트다)
- `cacheControl`은 Supabase 쪽 문제라 우리가 고칠 수 없었고, 프록시로 우회했다는 것
- 되돌릴 수 없어서 **일부러 하지 않은 것**(작업 3)과 그 이유
- 재발을 막는 장치(회귀 테스트)가 무엇이고, 그것이 실제로 작동하는지 확인한 방법
