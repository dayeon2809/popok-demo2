...
/** Published database profiles only, without the legacy local demo fallback. */
export async function getPublishedDatabaseArtists(): Promise<Artist[]> {
  try {
    const supabase = getSupabaseServer();
    ...
    if (error) {
      console.error("[getPublishedDatabaseArtists] Supabase error:", error);
      return [];
    }
    const companyLikeTypes = new Set(["organization", "company", "project_group", "group"]);
    return (data || [])
      .map(mapArtistRowToArtist)
      .filter((artist) => !companyLikeTypes.has(String(artist.type || "")));
  } catch (error) {
    console.error("[getPublishedDatabaseArtists] Unexpected error:", error);
    return [];
  }
}
