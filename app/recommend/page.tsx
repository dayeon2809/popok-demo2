...
export default function RecommendPage() {
  ...
  const handleRecommend = () => {
    const published = (artistsData as unknown as Artist[]).filter(
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px" }}>
      ...
      {/* ── 향후 AI 추천 확장용 로드맵 안내 주석 ── */}
      {/* 
        ROADMAP FOR FUTURE AI-BASED RECOMMENDATIONS:
        향후에는 규칙(Rule) 기반 매칭 대신 OpenAI/Gemini API 또는 임베딩 코사인 유사도 연산을 활용하여
        다음 데이터를 기반으로 정교한 시맨틱 AI 추천 모델로 확장할 예정입니다:
        - 방문자의 세부 설문 응답 및 자유로운 서술형 목적 (Context)
        - 안무가의 AI summary 및 대표작 설명 텍스트
        - Review Links 크롤링 원문 텍스트 분석
        - 아티스트 개별 세부 공연 일정 스케줄 매칭
        - 사용자의 북마크/저장/클릭 기록 등의 상호작용 피드백 로그 (Collaborative filtering)
      */}
    </div>
  );
}
