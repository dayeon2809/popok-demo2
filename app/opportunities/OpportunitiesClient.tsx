...
import { mockCollaborations, mockOpportunities, opportunityStats } from "@/lib/opportunities/mock-opportunities";
...
export default function OpportunitiesClient() {
  ...
  const visible = useMemo(() => mockOpportunities.filter((item) => matchesFilter(item, filter)), [filter]);
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.content}>
        ...
        <section aria-labelledby="collab-title" className={styles.section}>
          <div className={styles.sectionHeading}><div><p>COLLABORATION</p><h2 id="collab-title">협업 피드</h2></div><span>POPOK 아티스트의 제안</span></div>
          <div className={styles.collaborationList}>{mockCollaborations.map((post) => <article key={post.id} className={styles.collaborationCard}>
